                    ...
                    "default": """[
  {
    ...
    async def api_call(
        ...
        try:
            ...
            console_log += "[OK] Downloaded file\n"
            console_log += f"  Size: {len(file_bytes) / (1024*1024):.2f} MB\n"

            # Convert to tensor (try to load as image)
            # If output is PSD, we'll try to open it with PIL (which may not work for all PSDs)
            # For non-image outputs, we return a dummy 1x1 pixel tensor
            try:
                ...
                console_log += f"  Converted to tensor: {img_tensor.shape}\n"

            except Exception as e:
                console_log += f"  Warning: Could not convert output to image tensor: {e}\n"
                console_log += f"  Returning dummy 1x1 pixel tensor. Use output_url to access the file.\n"
                # Return a dummy 1x1 black pixel tensor

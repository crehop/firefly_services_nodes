"""
Adobe Photoshop Actions Node

Execute Photoshop actions (.atn files) on images using Adobe Photoshop API.
"""
...
from .photoshop_api import (
    ...
    PhotoshopAction,
    PhotoshopAsset,
    PhotoshopActionsOptions,
    PhotoshopActionsRequest,
    PhotoshopActionsResponse,
    PhotoshopActionsJobStatus,
)
...
class PhotoshopActionsNode:
    """
    Execute Photoshop actions on images using Adobe Photoshop API.

    Features:
    - Execute custom Photoshop actions (.atn files)
    - Support for patterns, fonts, and brushes
        ...
        return {
            "optional": {
                ...
                "action_url": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "tooltip": "URL to Photoshop action file (.atn)",
                }),
                "action_name": ("STRING", {
                    "default": "",
                    "multiline": False,
                    "tooltip": "Name of the action to execute from the .atn file",
                }),

                # Output settings
                "output_type": (
                    ["image/vnd.adobe.photoshop", "image/jpeg", "image/png", "image/webp"],
                    {
    ...
    def _build_debug_log(
        ...
        log = "=" * 55 + "\n"
        log += "POST /pie/psdService/photoshopActions\n"
        log += "-" * 55 + "\n"
        log += "Headers:\n"
        ...
        log += "  options:\n"
        log += "    actions:\n"
        log += f"      - href: {action_url}\n"
        log += "        storage: external\n"
        log += f"        actionName: {action_name}\n"
    ...
    async def api_call(
        ...
        """Execute Photoshop action on image using Photoshop API."""
        ...
        if not action_url:
            raise ValueError("Must provide 'action_url'")
        if not action_name:
            raise ValueError("Must provide 'action_name'")
        ...
        try:
            ...
            # Build request
            request = PhotoshopActionsRequest(
                inputs=[
                    PhotoshopActionsInput(
                        href=input_url,
                        storage="external"
                ...
                options=PhotoshopActionsOptions(
                    actions=[
                        PhotoshopAction(
                            storage="external",
                            href=action_url,
            ...
            submit_endpoint = ApiEndpoint(
                ...
                request_model=PhotoshopActionsRequest,
                response_model=PhotoshopActionsResponse,
            )
            ...
            console_log += "  Note: Photoshop Actions can take several minutes\n"
            ...
            poll_endpoint = ApiEndpoint(
                ...
                response_model=PhotoshopActionsJobStatus,
            )
            ...
            poll_op = PollingOperation(
                ...
                poll_interval=2.0,
                max_poll_attempts=300,  # 10 min timeout (actions can be slow)
                estimated_duration=30.0,  # Estimated 30s for Actions processing
                node_id=unique_id,
            )
            ...
            if not result.result or not result.result.outputs:
                ...
                raise Exception(f"No outputs returned from Photoshop Actions API. Check console logs or connect debug_log output for details.")

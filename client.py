        ...
        *req_meta,
        ...
        if status_code == 401:
            ...
            if isinstance(response_content, dict):
                ...
                # Show status code, URL, and error message for better debugging
                request_url = req_meta[1] if req_meta and len(req_meta) > 1 else "unknown"
                if response_content and len(response_content) < 200:  # Arbitrary limit for display
                    user_friendly = f"API Error {status_code}: {response_content} (URL: {request_url})"
                else:
                    error_msg = exc.message if hasattr(exc, 'message') and exc.message else "No response"
                    user_friendly = f"API Error {status_code}: {error_msg} (URL: {request_url})"
